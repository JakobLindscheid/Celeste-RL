...
class ReplayBuffer:
    ...
    def __init__(self, size, action_size, state_size, image_size, file_save=None):
        self.size = size
        ...
        self.next_states = torch.zeros((size+1, state_size), device=self.device)
        self.actions_probs = torch.zeros((size, action_size*2), device=self.device)
        ...
        if self.image_size is not None:
            self.images = torch.zeros((size+1, image_size[0], image_size[1], image_size[2]), device=self.image_device, dtype=torch.float16)
        if self.image_size is not None:
            self.next_images = torch.zeros((size+1, image_size[0], image_size[1], image_size[2]), device=self.image_device, dtype=torch.float16)
        self.index = 0
        self.current_size = 0
    ...
    def insert_data(self, state, new_state, image, new_image, actions_probs, reward, terminated,):
        ...
        self.next_states[self.index] = torch.tensor(new_state, device=self.device)
    ...
    def sample_data(self, batch_size):
        ...
        if self.image_size is not None:
            sampled_images = self.images[sampled_index.to(self.image_device)].to(device=self.device, dtype=torch.float32)
            sampled_new_images = self.next_images[sampled_index.to(self.image_device)].to(device=self.device, dtype=torch.float32)
        else:
            sampled_images = None
